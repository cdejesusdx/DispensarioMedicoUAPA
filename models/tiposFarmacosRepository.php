<?php
    class tiposFarmacosRepository {
        ...
        public function create($descripcion, $estado)
        {
            try
            {
                 $statement = $this->db->prepare("CALL InsTipoFarmaco (?,?);");
        ...
        public function update($id, $descripcion, $estado)
        {
            try
            {
                 $statement = $this->db->prepare("CALL ActTipoFarmaco (?,?,?);");
        ...
        public function delete($id)
        {
            try
            {
                 $statement = $this->db->prepare("CALL DelTipoFarmaco (?);");
                 $statement->bindParam(1, $id);
                ...
                while($row = $statement->fetch(PDO::FETCH_ASSOC))
                {
                    ?>
                        <tr class="<?php print($row['Estado']) == 'Activa' ? '' : 'danger';?>">
                            ...
                            <td><?php print($row['IdTipoFarmaco']); ?></td>
                            <td><?php print($row['Descripcion']); ?></td>
                            <td style="text-align: center;"><?php print($row['Estado']); ?></td>
                           
                            <td style="text-align: center;">
                                <a href="update.php?updateId=<?php print($row['IdTipoFarmaco']); ?>"><i class="glyphicon glyphicon-edit"></i></a>
                            </td>
                            
                            <td style="text-align: center;">
                                <a href="delete.php?deleteId=<?php print($row['IdTipoFarmaco']); ?>" onClick="return confirm('¿Está seguro que desea eliminar este registro?');" >
