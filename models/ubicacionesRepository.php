<?php
    class ubicacionesRepository {
        ...
        public function create($descripcion, $estado)
        {
            try
            {
                 $statement = $this->db->prepare("CALL InsUbicaciones (?,?);");
        ...
        public function update($id, $descripcion, $estado)
        {
            try
            {
                 $statement = $this->db->prepare("CALL ActUbicacion (?,?,?);");
        ...
        public function delete($id)
        {
            try
            {
                 $statement = $this->db->prepare("CALL DelUbicacion (?);");
                 $statement->bindParam(1, $id);
                ...
                while($row = $statement->fetch(PDO::FETCH_ASSOC))
                {
                    ?>
                        <tr class="<?php print($row['Estado']) == 'Activa' ? '' : 'danger';?>">
                            ...
                            <td><?php print($row['IdUbicacion']); ?></td>
                            <td><?php print($row['Descripcion']); ?></td>
                            <td><?php print($row['Estante']); ?></td>
                            <td><?php print($row['Tramo']); ?></td>
                            <td><?php print($row['Casilla']); ?></td>
                            <td style="text-align: center;"><?php print($row['Estado']); ?></td>
                           
                            <td style="text-align: center;">
                                <a href="update.php?updateId=<?php print($row['IdUbicacion']); ?>"><i class="glyphicon glyphicon-edit"></i></a>
                            </td>
                            
                            <td style="text-align: center;">
                                <a href="delete.php?deleteId=<?php print($row['IdUbicacion']); ?>" onClick="return confirm('¿Está seguro que desea eliminar este registro?');" >
